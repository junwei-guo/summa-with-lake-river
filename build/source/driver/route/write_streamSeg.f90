...
! data types
USE nrtype_mizuroute,    only : i4b,dp,lgt
USE nrtype_mizuroute,    only : strLen               ! string length
...
! named variables
USE var_lookup_route,only:ixStruct, nStructures  ! index of data structures
USE var_lookup_route,only:ixDims,   nDimensions  ! index of dimensions
USE var_lookup_route,only:ixHRU,    nVarsHRU     ! index of variables for the HRUs
USE var_lookup_route,only:ixSEG,    nVarsSEG     ! index of variables for the stream segments
USE var_lookup_route,only:ixHRU2SEG,nVarsHRU2SEG ! index of variables for the hru2segment mapping
USE var_lookup_route,only:ixNTOPO,  nVarsNTOPO   ! index of variables for the network topology
USE var_lookup_route,only:ixPFAF,   nVarsPFAF    ! index of variables for the pfafstetter code
