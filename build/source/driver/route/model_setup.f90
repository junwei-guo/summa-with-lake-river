...
CONTAINS
 ...
 SUBROUTINE init_model(cfile_name, ierr, message)
  ...
  USE popMetadat_module_mizuroute,   ONLY : popMetadat       ! populate metadata
 ...
 subroutine init_data(ierr, message)
  ...
  USE var_lookup_route,  ONLY : ixHRU2SEG              ! index of variables for data structure
  USE var_lookup_route,  ONLY : ixNTOPO                ! index of variables for data structure
...
! *********************************************************************
 ...
 SUBROUTINE init_ntopo(nHRU_out, nRch_out,                                           & ! output: number of HRU and Reaches
  ...
  USE var_lookup_route,           ONLY : ixPFAF                   ! index of variables for the pfafstetter code
  ...
  if (meta_PFAF(ixPFAF%code)%varFile .and. (meta_PFAF(ixPFAF%code)%varName .ne.'empty')) then
